...
aug_list = ["na", "shuffle", "jit_scal", "perm_jit", "resample", "noise", "scale", "negate", "t_flip", "rotation", "perm", "t_warp", "hfc", "lfc", "p_shift", "ap_p", "ap_f"]
...
def resample(x):
    """
    Resamples the input array `x` by interpolating and selecting specific indices.

    This function performs the following steps:
    1. Interpolates the input array along its second axis using cubic interpolation.
    2. Generates a new set of indices based on a random starting point and a step size.
    3. Selects the interpolated values at the generated indices.

    Parameters:
    ----------
    x : numpy.ndarray
        A 3D array of shape (n_samples, n_features, n_channels) where the second axis
        represents the features to be resampled.

    Returns:
    -------
    numpy.ndarray
        A resampled array of the same shape as the input `x`.

    Notes:
    -----
    - The interpolation is performed using `scipy.interpolate.interp1d`.
    - The resampling indices are generated to ensure the output has the same shape
      as the input along the second axis.
    - The function assumes that the input array `x` has at least two dimensions.

    Raises:
    ------
    ValueError
        If the input array `x` does not have the required dimensions.

    Example:
    --------
    >>> import numpy as np
    >>> x = np.random.rand(10, 100, 3)  # Example input
...
def generate_high(sample, r, high=True):
    ...
    fd = torch.unsqueeze(fd, 0) if len(fd.shape) != len(sample.shape) else fd
    return fft, fd
...
def ifft_phase_shift(sample):
    ...
    ifft = torch.unsqueeze(ifft, 0) if len(ifft.shape) != len(sample.shape) else ifft
    return ifft
